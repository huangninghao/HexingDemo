...
public class HXHdlcDLMSFrame {
    ...
    public byte[] getHLSAuthAARQFrame(HXFramePara fPara) {
        ...
        TmpArr[Index++] = 0x02;
        if (fPara.enLevel != 0x00) {
            if (fPara.encryptionMethod == HXFramePara.AuthMethod.AES_GCM_128) {
                TmpArr[Index++] = 0x05;// AES-GCM-128,带8个字节随机数
            } else if (fPara.encryptionMethod == HXFramePara.AuthMethod.MD5) {
                TmpArr[Index++] = 0x03;
            }
            TmpArr[Index++] = (byte) 0xac;
            TmpArr[Index++] = 0x0a;
        ...
        plaintByte[indexPlain++] = (byte) 0xff;
        if (fPara.enLevel != 0x00) {
            cByte = buildAPDU(plaintByte, HXFramePara.APDUtype.glo_initiateRequest, fPara);
     ...
     * @return
     */
    private byte[] buildAPDU(byte[] plaintByte, HXFramePara.APDUtype operType,
                             HXFramePara fpara) {
        byte[] rtnBytes;
        ...
        if (fpara.enLevel == 0x00) {
            ...
            if (fpara.enLevel == 0x10)// TAG||LEN||SH||APDU||T
            {
                ...
                byte[] tmpEncryBytes = gcmEncryptMessage(fpara.enKey,
    ...
    public String getStoc(HXFramePara fpara, byte[] RecArr) {
        ...
        if (fpara.enLevel == 0x00) {
            ...
            stringBuilder.append("0910");
            for (int i = 0; i < 16; i++) {
                if (EncodeNum[i] < 0x10) {
                    stringBuilder.append("0");
                }
                stringBuilder.append(Integer.toString(EncodeNum[i], 16));
            }
            ...
            if (fpara.encryptionMethod == HXFramePara.AuthMethod.AES_GCM_128) {
                ...
                tag = new byte[12];
                System.arraycopy(tmpEncryBytes, msg.length, tag, 0, tag.length);
                stringBuilder.append("0911");
                stringBuilder.append("10");// SC
                for (int i = 8; i < 12; i++)// FC
                {
                    if ((iv[i] & 0xff) < 0x10) {
                        stringBuilder.append("0");
                    }
                    stringBuilder.append(Integer.toString((iv[i] & 0xff), 16));
                }
                for (int i = 0; i < 12; i++)// T
                {
                    ...
                    stringBuilder.append((Integer.toString((tag[i] & 0xff), 16)));
                }
                return stringBuilder.toString();

            }
            if (fpara.encryptionMethod == HXFramePara.AuthMethod.MD5) {
                // MD5身份验证
                MessageDigest md;
                stringBuilder.append("0910");
                try {
                    ...
                    for (int i = 0; i < 16; i++) {
                        ...
                        stringBuilder.append(Integer.toString(Myresult[i], 16));
                    }
    ...
    public byte[] getReadRequestNormalFrame(HXFramePara fPara) {
        ...
        if (fPara.WriteData != null && fPara.WriteData.length() > 0) {
            fPara.OBISattri = fPara.OBISattri.substring(0, fPara.OBISattri.length() - 2);
            fPara.OBISattri += fPara.WriteData;
        }
        for (int i = 0; i < fPara.OBISattri.length() / 2; i++) {
            tmpPlain[indexPlain++] = (byte) Integer.parseInt(fPara.OBISattri.substring(i * 2, i * 2 + 2), 16);
        }
        ...
        if (fPara.enLevel != 0x00) {
            cByte = buildAPDU(plaintByte, HXFramePara.APDUtype.glo_get_request, fPara);
            for (int i = 0; i < cByte.length; i++) {
                TmpArr[Index++] = cByte[i];
            }
        } else {
        ...
        TmpArr[2] = (byte) (Index + 1);
        byte[] checkByte = CRC16(TmpArr, 1, 4 + fPara.DestAddr.length);
        TmpArr[HCSindex] = checkByte[0];
        TmpArr[HCSindex + 1] = checkByte[1];
        checkByte = CRC16(TmpArr, 1, Index - 1);
        TmpArr[Index++] = checkByte[0];
    ...
    public byte[] getReadRequestBlockFrame(HXFramePara fPara) {
        ...
        plaintByte = Arrays.copyOf(tmpPlain, indexPlain);
        if (fPara.enLevel != 0x00) {
            cByte = buildAPDU(plaintByte, HXFramePara.APDUtype.glo_get_request, fPara);
            for (int i = 0; i < cByte.length; i++) {
                TmpArr[Index++] = cByte[i];
    ...
    public byte[] getWriteRequestNormalFrame(HXFramePara fPara) {
        ...
        if (fPara.enLevel != 0x00) {
            cByte = buildAPDU(plaintByte, HXFramePara.APDUtype.glo_set_request, fPara);
            for (int i = 0; i < cByte.length; i++) {
                TmpArr[Index++] = cByte[i];
            }
        } else {
     ...
     * @return byte[]
     */
    public byte[] getWriteRequestBlockFrame(HXFramePara fPara) {
        int Index = 0;
        int HCSindex = 0;
        byte[] TmpArr;
        byte[] CheckByte = new byte[2];
        ...
        int indexPlain = 0;
        byte[] cByte;
        Boolean IsLastBlock = (fPara.WriteData.trim().length() / 2 + 20) <= fPara.MaxSendInfo_Value;
        TmpArr = new byte[300];
        Index = 0;
        TmpArr[Index++] = 0x7E;
        TmpArr[Index++] = (byte) 0xA0;
        ...
        tmpPlain[indexPlain++] = (byte) (!IsLastBlock ? 0x00 : 0xff);// last
        // block
        // or
        // not
        tmpPlain[indexPlain++] = (byte) (fPara.BlockNum / (256 * 256 * 256));// block_number
    ...
    public byte[] getActionRequestNormalFrame(HXFramePara fPara) {
        ...
        // #endregion
        if (fPara.enLevel != 0x00) {
            cByte = buildAPDU(plaintByte, HXFramePara.APDUtype.glo_action_request, fPara);
            for (int i = 0; i < cByte.length; i++) {
                TmpArr[Index++] = cByte[i];
            }
        } else {
     ...
     * @return []
     */
    public byte[] getChangeChannel(HXFramePara para) {
        String headData = "7E000508014348";
        int channel = -1;
        if (para.isFixedChannel >= 0) {
            channel = para.isFixedChannel;
        } else {
            String meterNumber = para.getStrMeterNo();
            int res = 0;
    ...
    public byte[] CRC16(byte[] data, int startIndex, int endIndex) {
        ...
        while (startIndex <= endIndex) {
            int Temp = CRCval & 0x00ff;
            CRCval = CRC16Tab[Temp ^ (data[startIndex] & 0xff)] ^ (CRCval >> 8);
            startIndex++;
        }

        CRCval = CRCval ^ 0xFFFF;
        int Temp1 = CRCval & 0x00FF;
        int Temp2 = CRCval & 0xFF00;
        Temp2 = Temp2 >> 8;
     ...
     * @param iv      ：初始化向量
     * @param iv_len  int
     * @param hdr     ：验证附加信息ADD（10+AK）
     * @param hdr_len int
     * @param msg     ：输出-密文C
     * @param tag     ：输出-消息验证码Mac
     */
    private byte[] gcmEncryptMessage(byte[] key, int key_len, byte[] iv,
     ...
     * @param tag_len int
     */
    private void gcmDecryptMessage(byte[] key, int key_len, byte[] iv,
    ...
    public byte[] getOriginalData(byte[] cText, HXFramePara fpara) {
        ...
        if (recAuEnByte == 0x20) {
            ...
            gcmDecryptMessage(fpara.enKey, (byte) fpara.enKey.length, iv,
        ...
        if (recAuEnByte == 0x30) {
            ...
            gcmDecryptMessage(fpara.enKey, fpara.enKey.length, iv, iv.length,

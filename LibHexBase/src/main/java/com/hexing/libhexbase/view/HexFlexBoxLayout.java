 ...
 * @author caibinglong
 *         date 2018/8/9.
 *         desc 自适应 布局 动态添加view 自动换行显示
 */

public class HexFlexBoxLayout extends ViewGroup {

    private int mScreenWidth;
    private int horizontalSpace = 0, verticalSpace = 0;
    private float mDensity;//设备密度，用于将dp转为px

    public HexFlexBoxLayout(Context context) {
        this(context, null);
    }

    public HexFlexBoxLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        //获取屏幕宽高、设备密度
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode != MeasureSpec.EXACTLY) {
            widthSize = mScreenWidth + getPaddingLeft() + getPaddingRight();//如果没有指定宽，则默认为屏幕宽
        }
